...
- **Node Helper**: Added `node.getSlotsByGroup(groupName)` to easily retrieve related slots.
- **Strict Group Validation**: Added `enforceSlotGroups` option to NodeGraph. When enabled, connections are restricted to slots that share the same `group` value.
- **Flexible Connections**: Added `enforceDirection` option (default `true`). When set to `false`, allows connecting Input-to-Input or Output-to-Output.
